<template>
	<view class="item">
		...
		<view class="admin-item">		
			...
			<view class="admin-item-list" v-for="item in itemProList" :kye="item.i_id" style="border: 4px rgba(222, 222, 222, .3) solid;border-radius: 40rpx;box-shadow: 10px 10px 5px #d9d9d9;">
				...
				<view style="display:flex;flex-direction: raw;margin: 30rpx 0;">
					...
					<view style="margin-top: 10rpx;color: #cf7f06;">状态：{{item.i_progress}} <u-icon @click="toTimeLine(item)" name="file-text" color="#cf7f06" size="34"></u-icon></view>
				</view>
...
<script>
	export default{
		...
		methods: {
			...
			async getProItemList(){
				this.itemProList = await this.$u.get('/admin/items/list2');//这是一个耗时的ajax请求,await表是一直阻塞，直到获取响应数据
			},

<template>
	<view class="user">
		...
		<view class="user-list">
			...
			<view v-if="userList.length!=0" class="user-list-item" v-for="user in userList" :kye="user.u_id" style="border: 4px rgba(222, 222, 222, .3) solid;border-radius: 40rpx;box-shadow: 10px 10px 5px #d9d9d9;">
...
<script>
	export default{
		...
		onLoad(params){
			//获取传递的json字符串
			this.form = JSON.parse(decodeURIComponent(params.ditem));
		...
		methods: {
			async getUserList(){
				this.userList = await this.$u.post('admin/department/listasd',this.form);//这是一个耗时的ajax请求,await表是一直阻塞，直到获取响应数据
			},
